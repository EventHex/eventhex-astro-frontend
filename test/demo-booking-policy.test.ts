...
test("requires approval at the 22:00 IST boundary", () => {
  assert.equal(requiresDemoApproval("2026-01-01T16:30:00.000Z"), true);
});

test("does not require approval immediately before 22:00 IST", () => {
  assert.equal(requiresDemoApproval("2026-01-01T16:29:00.000Z"), false);
});

test("requires approval at 02:00 IST", () => {
  assert.equal(requiresDemoApproval("2026-01-01T20:30:00.000Z"), true);
});

test("does not require approval at the 02:30 IST boundary", () => {
  assert.equal(requiresDemoApproval("2026-01-01T21:00:00.000Z"), false);
});
