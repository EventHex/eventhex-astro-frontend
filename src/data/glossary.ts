...
<h2>How does on-demand badge printing work?</h2>
<p>The attendee scans their registration QR code (from email or the event app) at a check-in kiosk or staffed counter. The check-in system verifies the ticket, pulls the attendee record, and sends a print job to a connected thermal or inkjet badge printer. The whole cycle — scan, verify, print, collect — takes 6–10 seconds per attendee, and multiple kiosks run in parallel to keep queues moving.</p>
<h2>What hardware does event badge printing need?</h2>
<p>A typical on-demand setup needs three things per station: a tablet or laptop running the check-in software, a badge printer (common choices are thermal card printers for PVC badges or dye-sublimation and inkjet units for full-colour paper stock), and badge consumables (stock, ribbons, holders or lanyards). Venue Wi-Fi is the usual weak point, so look for check-in software with an offline mode that queues print jobs locally and syncs later.</p>
<h2>Pre-printed vs on-demand badges: which is better?</h2>
<p>Pre-printing suits small events with stable attendee lists, but it wastes badges on no-shows (typically 20–40% at free events), breaks on walk-ins and late registrations, and consumes staff days on sorting. On-demand printing costs slightly more per badge but removes waste, handles same-day changes, and turns check-in itself into the badge distribution point — which is why most conferences above a few hundred attendees have moved on-demand.</p>`,
    related: [
      "qr-code-check-in",
      "event-check-in-software",
      "event-registration-software",
